...
// a hash table, and only those blocks are filled with the scalar field and meshed. Each block
// the surface crosses gets one vertex per crossed cell and one quad per crossing edge, joined
// across block borders through the table. Vertices come out in the vessel's frame, so the mesh
// turns with the vessel between extractions.
...
@group(3) @binding(6) var<storage, read_write> table: array<atomic<u32>>;
@group(3) @binding(7) var<storage, read_write> table_index: array<u32>;
@group(3) @binding(8) var<storage, read_write> blocks: array<u32>;
@group(3) @binding(9) var<storage, read_write> dispatch: array<u32>;
...
/// One workgroup per block for the kernels that follow.
@compute @workgroup_size(1)
fn prepare_dispatch() {
    dispatch[0] = min(atomicLoad(&counters[COUNTER_BLOCKS]), surface.max_blocks);
    dispatch[1] = 1u;
    dispatch[2] = 1u;
}
...
/// Splatted density and foam at every corner of a block: the smooth kernel of every particle
/// within reach.
@compute @workgroup_size(125)
fn density(@builtin(workgroup_id) group: vec3<u32>, @builtin(local_invocation_index) t: u32) {
    let block = group.x;
    let b = block_coords(blocks[block]);
    let corner = vec3<i32>(i32(t / 25u), i32((t / 5u) % 5u), i32(t % 5u));
    ...
    corners[block * CORNERS_PER_BLOCK + t] = vec2(d, f);
}

fn corner_at(block: u32, c: vec3<i32>) -> vec2<f32> {
    return corners[block * CORNERS_PER_BLOCK + u32((c.x * 5 + c.y) * 5 + c.z)];
}

fn vertex_slot(block: u32, c: vec3<i32>) -> u32 {
    return block * CELLS_PER_BLOCK + u32((c.x * 4 + c.y) * 4 + c.z);
}
...
@compute @workgroup_size(64)
fn place_vertices(@builtin(workgroup_id) group: vec3<u32>, @builtin(local_invocation_index) t: u32) {
    let block = group.x;
    let b = block_coords(blocks[block]);
    let cell = vec3<i32>(i32(t / 16u), i32((t / 4u) % 4u), i32(t % 4u));
    let slot_out = vertex_slot(block, cell);
    var d: array<f32, 8>;
    var mask = 0u;
    var foam = 0.0;
    var weight = 0.0;
    for (var k = 0u; k < 8u; k++) {
        let corner = corner_at(block, cell + OFFSETS[k]);
        d[k] = corner.x;
        foam += corner.y;
        weight += corner.x;
    ...
    if (mask == 0u || mask == 255u) {
        cell_vertex[slot_out] = NO_VERTEX;
        return;
    }
    var sum = vec3(0.0);
    var crossings = 0.0;
...
fn vertex_of(block: u32, b: vec3<i32>, cell: vec3<i32>) -> u32 {
    ...
    if (all(shift == vec3(0))) {
        return cell_vertex[vertex_slot(block, cell)];
    }
    let other = find_block(b + shift);
    if (other == NO_BLOCK) {
        return NO_VERTEX;
    }
    return cell_vertex[vertex_slot(other, cell - shift * BLOCK)];
}
...
@compute @workgroup_size(64)
fn place_quads(@builtin(workgroup_id) group: vec3<u32>, @builtin(local_invocation_index) t: u32) {
    let block = group.x;
    let b = block_coords(blocks[block]);
    let c = vec3<i32>(i32(t / 16u), i32((t / 4u) % 4u), i32(t % 4u));
    let inside = corner_at(block, c).x > surface.iso;
    if ((corner_at(block, c + vec3(1, 0, 0)).x > surface.iso) != inside) {
        emit_quad(vec4(
            vertex_of(block, b, c),
    ...
    if ((corner_at(block, c + vec3(0, 1, 0)).x > surface.iso) != inside) {
        emit_quad(vec4(
            vertex_of(block, b, c),
            vertex_of(block, b, c + vec3(0, 0, -1)),
    ...
    if ((corner_at(block, c + vec3(0, 0, 1)).x > surface.iso) != inside) {
        emit_quad(vec4(
            vertex_of(block, b, c),
            vertex_of(block, b, c + vec3(-1, 0, 0)),
